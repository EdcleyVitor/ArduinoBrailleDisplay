...
class ScanScreen extends StatefulWidget {
  ...
  const ScanScreen({super.key, required this.bleService, this.isAdditional = false});
...
class _ScanScreenState extends State<ScanScreen> {
  ...
  void initState() {
    super.initState();
    _connectedIds = widget.bleService.connectedDevices
        .where((d) => d.isReady)
        .map((d) => d.id)
        .toSet();
        ...
        setState(() {
          _results = results.where((r) {
            final hasService = r.advertisementData.serviceUuids
                .any((u) => u.toString().toLowerCase() == BrailleBluetoothService.serviceUUID.toLowerCase());
  ...
  void _connectToDevice(BluetoothDevice device) async {
    final alreadyConnected = _connectedIds.contains(device.remoteId.toString());
    if (alreadyConnected) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Ja conectado a este dispositivo!', style: GoogleFonts.poppins()),
    ...
    if (mounted) {
      ...
      if (success) {
        if (widget.isAdditional) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('${device.platformName} conectado!', style: GoogleFonts.poppins()),
  ...
  bool _isAlreadyConnected(BluetoothDevice device) {
    return _connectedIds.contains(device.remoteId.toString());
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isAdditional ? 'Conectar Mais Dispositivos' : 'Escanear Dispositivos',
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final name = device.platformName.isNotEmpty
                              ? device.platformName
                              : 'Desconhecido';
                          final alreadyConnected = _isAlreadyConnected(device);
                          return Card(
                            ...
                            child: ListTile(
                              leading: Icon(
                                ...
                                color: alreadyConnected ? const Color(0xFF4CAF50) : const Color(0xFF6C63FF),
                              ),
                              title: Row(
                                children: [
                                  Expanded(
                                  ...
                                  if (alreadyConnected)
                                    Container(
                                      ...
                                      child: Text(
                                        'CONECTADO',
                                        style: GoogleFonts.poppins(fontSize: 10, fontWeight: FontWeight.w700, color: const Color(0xFF4CAF50)),
                                      ),
                                    ),
                                ],
                              ),
                              ...
                              trailing: Icon(
                                alreadyConnected ? Icons.check_circle : Icons.chevron_right_rounded,
                                color: alreadyConnected ? const Color(0xFF4CAF50) : null,
                              ),
                              onTap: alreadyConnected ? null : () => _connectToDevice(device),
                            ),
                          );
                        },
                      ),
