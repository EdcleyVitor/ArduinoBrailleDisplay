/*
  BrailleBridge v9 - ESP32 comum + 6 LEDs
  ==========================================
  ...
  Sincronizacao multi-dispositivo:
    O app aguarda delayExibicao + delayPausa + margem antes de enviar
    a proxima letra. Nao depende de @DONE para timing.
    O firmware descarta padroes recebidos enquanto esta exibindo.
*/
...
#define BLE_CHAR_SIGN_UUID   "6e400004-b5a3-f393-e0a9-e50e24dcca9e"

#define BRIDGE_SIGNATURE "WhatGodWrought1844"
...
void mostrarPadrao(String padrao) {
  testeAtivo = false;
  bool leds[6];
  if (!parsePadrao(padrao, leds)) return;
...
void iniciarBLE() {
  ...
  BLECharacteristic *signChar = svc->createCharacteristic(
    BLE_CHAR_SIGN_UUID,
    BLECharacteristic::PROPERTY_READ
  );
  signChar->setValue(BRIDGE_SIGNATURE);
